...
if(!empty($_SESSION["Username"])){
  ...
  echo "<title>All cheque Requests</title>";
    include "static/adminnav.php" ;
    if(!empty($_POST["Authenticate"])){
      $key = $_POST["t1"];
      $modify = "UPDATE cheque SET Status ='Delievered' WHERE Account_number='$key'";
      $res =$connection->query($modify);
      }
      $res2 =mysqli_query($connection , "SELECT Account_number ,Num_of_cheques,Request_date FROM cheque WHERE Status='Pending'");
...
<?php
        ...
        while($result =mysqli_fetch_array($res2)){
          ...
          $b = $result["Num_of_cheques"];
          $c =$result["Request_date"];
            ++$i;
            $c =substr($c,0,10);
          echo" <tr>
                  ...
                  <td>$b</td>
                  <td>$c</td>
                  <td><input type='submit' class='simplebutton' value='Delievered' name='Authenticate'></td>
                  </form>
                </tr>";
         }
         echo "</table> </div>";
